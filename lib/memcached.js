...
MemcachedDatabank.prototype.index = function(type, id, value, callback) {
    ...
    Step(
        function() {
            ...
            bank.append('databank-type-keys', type, key, this);
        },
        function(err, allKeys) {
            ...
            if (bank.hasOwnProperty('schema') &&
                ...
                for (i = 0; i < indices.length; i++) {
                    ...
                    bank.append('databank-type-index', type + ':' + k, key, group());
                }
            }
        },
...
MemcachedDatabank.prototype.deindex = function(type, id, callback) {
    ...
    Step(
        ...
        function(err, results) {
            ...
            value = results;
            // XXX: make sure we don't recurse here
            bank.remove('databank-type-keys', type, key, this);
        },
        function(err, allKeys) {
            var i = 0, group = this.group(), indices, k;
            if (err) throw err;
            if (bank.hasOwnProperty('schema') &&
                ...
                for (i = 0; i < indices.length; i++) {
                    k = Databank.deepProperty(value, indices[i]);
                    bank.remove('databank-type-index', type + ':' + k, key, group());
                }
            }
        },
        function(err, resultses) {
