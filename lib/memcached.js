...
MemcachedDatabank.prototype.create = function(type, id, value, callback) {
    ...
    Step(
        function() {
            bank.client.add(key, JSON.stringify(value), bank.expire, this);
        },
        function(err, result) {
            if (err) throw err;
...
MemcachedDatabank.prototype.update = function(type, id, value, callback) {
    ...
    Step(
        ...
        function(err) {
            if (err) throw err;
            bank.client.replace(key, JSON.stringify(value), bank.expire, this);
        },
        function(err, result) {
            if (err) throw err;
...
MemcachedDatabank.prototype.save = function(type, id, value, callback) {
    ...
    Step(
        ...
        function(err) {
            if (err) throw err;
            bank.client.set(key, JSON.stringify(value), bank.expire, this);
        },
        function(err, result) {
            if (err) throw err;
            if (!result) throw new NoSuchThingError(type, id);
...
MemcachedDatabank.prototype.index = function(type, id, value, callback) {
    ...
    Step(
        function() {
            // XXX: make sure we don't recurse here
            bank.append('_databank_keys', type, key, this);
        },
        function(err, allKeys) {
            ...
            if (bank.hasOwnProperty('schema') &&
                ...
                for (i = 0; i < indices.length; i++) {
                    ...
                    bank.append('_databank_index', type + ':' + k, key, group());
                }
            }
        },
...
MemcachedDatabank.prototype.deindex = function(type, id, callback) {
    ...
    Step(
        ...
        function(err, results) {
            ...
            value = results;
            // XXX: make sure we don't recurse here
            bank.remove('_databank_keys', type, key, this);
        },
        function(err, allKeys) {
            var i = 0, group = this.group(), indices, k;
            if (err && !(err instanceof NoSuchThingError)) throw err;
            if (bank.hasOwnProperty('schema') &&
                bank.schema.hasOwnProperty(type) &&
                bank.schema[type].hasOwnProperty('indices')) {
                indices = bank.schema[type].indices;
                for (i = 0; i < indices.length; i++) {
                    k = Databank.deepProperty(value, indices[i]);
                    bank.remove('_databank_index', type + ':' + k, key, group());
                }
            }
        },
        function(err, resultses) {
        ...
        Step(
            function() {
                ...
                for (property in indexed) {
                    bank.read('_databank_index', type + ':' + indexed[property], group());
                }
            },
